...
local sprite      = Resources.sprite_load("DeerItems", "item/TwistedOpinion", PATH.."assets/sprites/items/sRedItems/TwistedOpinion.png", 1, 18, 17)
local skullSprite = Resources.sprite_load("DeerItems", "particle/TwistedOpinionSkull", PATH.."assets/sprites/particle/TwistedOpinionSkull.png", 1, 16, 16)
local burstSprite = Resources.sprite_load("DeerItems", "particle/TwistedOpinionBurst", PATH.."assets/sprites/particle/TwistedOpinionBurst.png", 10, 16, 16)
local barSprite   = Resources.sprite_load("DeerItems", "particle/TwistedOpinionCustomBar", PATH.."assets/sprites/particle/TwistedOpinionCustomBar.png", 1, 0, 0)
local sndLaunch   = Resources.sfx_load("DeerItems", "TwistedOpinion/launch", PATH.."assets/sounds/TwistedOpinionLaunch.ogg")
local sndImpact   = Resources.sfx_load("DeerItems", "TwistedOpinion/impact", PATH.."assets/sounds/TwistedOpinionImpact.ogg")
...
local HUD_FILL_FG   = Color(0xc060ff)

-- Настройки баланса: вынесены отдельно, чтобы их было проще менять.
...
local FIRE_COOLDOWN  = 6     -- мин. интервал между выстрелами (кадры) — защита от спама при огромном регене

-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║  Объект «череп» — самонаводящийся снаряд (по образцу AtGMissileMk0)         ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝
local oSkull = Object.new("DeerItems", "TwistedOpinionSkull")
...
-- ╚═══════════════════════════════════════════════════════════════════════════╝
local item = Item.new("DeerItems", "TwistedOpinion")
...
item:onPostStep(function(actor, stack)
    ...
    local data = actor:get_data("TwistedOpinion", GUID)
...
gm.post_script_hook(gm.constants.draw_hud, function()
    ...
    if (p:item_stack_count(item, Item.STACK_KIND.any) or 0) <= 0 then return end

    local threshold = p.maxhp * THRESHOLD_FRAC
    if threshold <= 0 then return end

    local data = p:get_data("TwistedOpinion", GUID)
    local frac = math.min((data.soul or 0) / threshold, 1)
    ...
    if fill > 0 then
        gm.draw_set_colour(HUD_FILL_FG)
