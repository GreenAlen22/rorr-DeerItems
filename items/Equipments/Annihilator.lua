...
oCharge:onStep(function(self)
    -- Заряд рассчитывает хост. Клиенты только рисуют снаряд, синхронизированный через
    -- projectile_sync, и не двигают вторую локальную копию.
...
equip:onUse(function(actor)
    -- Коллбек снаряжения вызывается у всех. Создаём заряд только на хосте, чтобы
    -- синхронизированный projectile_sync был единственной копией, видимой клиентам.
