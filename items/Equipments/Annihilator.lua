...
local sprite       = Resources.sprite_load("DeerItems", "equipment/Annihilator", PATH.."assets/sprites/items/sEquipments/Annihilator.png", 1, 16, 16)
local chargeSprite = Resources.sprite_load("DeerItems", "particle/AnnihilatorCharge", PATH.."assets/sprites/particle/AnnihilatorCharge.png", 1, 8, 8)
local exploSprite  = Resources.sprite_load("DeerItems", "particle/AnnihilatorBoom", PATH.."assets/sprites/particle/AnnihilatorBoom.png", 1, 8, 8)
local zapSprite    = Resources.sprite_load("DeerItems", "particle/AnnihilatorZap", PATH.."assets/sprites/particle/AnnihilatorZap.png", 1, 8, 8)
...
local TENDRIL_RANGE  = 220       -- радиус действия молний, px
local TENDRIL_EVERY  = 6         -- частота молний, кадры (10 раз/сек)
local TENDRIL_TARGETS= 4         -- сколько целей бьёт за тик
local TENDRIL_COEF   = 0.6       -- урон молнии за тик (0.6×10/сек = 600%/сек на цель)
local EXPLO_COEF     = 40.0      -- урон взрыва = 4000% урона игрока (коэффициент)
local EXPLO_SIZE     = 20 * 32   -- размер взрыва, px (20 м ≈ 640px)
local CONTACT_RADIUS = 30        -- дистанция «контакта» с врагом для подрыва, px
