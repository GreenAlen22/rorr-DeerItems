-- Точка входа мода: подключает зависимости, загружает контент и применяет настройки предметов.
...
-- Возвращает имя файла без пути, чтобы сопоставить его с записью в манифесте.
local function file_basename(file_path)
    return file_path:gsub("\\", "/"):match("([^/]+)$")
end

-- Загружает файл по имени, не полагаясь на порядок, в котором API вернул содержимое папки.
...
-- Собирает список предметов один раз, затем загружает все зарегистрированные Lua-файлы предметов.
...
-- Загружает все Lua-файлы из папки. Порядок папок задаётся ниже в Initialize.
...
-- Сначала регистрируем объекты, затем читаем настройки и отключаем запрещённые предметы из лута.
Initialize(function()
    require_folder(PATH.."Interactables")
    require_folder(PATH.."actor")
    require_folder(PATH.."elite")
